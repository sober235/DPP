...
if wave_vcc:
    ...
    netG = CovDecoder(nch_in=2, nch_h=256, nch_out = 2, layerNo=10, filter_size=3, in_size=[3,3], out_size=[nx,ny]).to(device)
    netK = CovDecoder(nch_in=2, nch_h=64, nch_out = 48, layerNo=5, filter_size=3, in_size=[3,3], out_size=[11,11]).to(device)
    init_weights(netG, init_type='normal', init_gain=1e-2)
    init_weights(netK, init_type='normal', init_gain=1e-2)
    paramsG = netG.parameters()
    paramsk = netK.parameters()
    params = itertools.chain(paramsG, paramsk) # 参数联合
    ncoil = 24
    ...
    netG = CovDecoder(nch_in=2, nch_h=256, nch_out =2, layerNo=10, filter_size=3, in_size=[3,3], out_size=[nx,ny]).to(device)
    netG2 = CovDecoder(nch_in=2, nch_h=256, nch_out = 96, layerNo=10, filter_size=3, in_size=[3,3], out_size=[nx,ny]).to(device)
    netK = CovDecoder(nch_in=2, nch_h=64, nch_out = 48, layerNo=5, filter_size=3, in_size=[3,3], out_size=[11,11]).to(device)
    netK2 = CovDecoder(nch_in=2, nch_h=64, nch_out = 48, layerNo=5, filter_size=3, in_size=[3,3], out_size=[11,11]).to(device)
    netP = CovDecoder(nch_in=2, nch_h=64, nch_out = 2, layerNo=5, filter_size=3, in_size=[3,3], out_size=[11,11]).to(device) # pf, cartesian
    init_weights(netG, init_type='normal', init_gain=1e-2)
    init_weights(netG2, init_type='normal', init_gain=1e-2)
    init_weights(netK, init_type='normal', init_gain=1e-6)
    init_weights(netK2, init_type='normal', init_gain=1e-6)
    init_weights(netP, init_type='normal', init_gain=1e-6)
    paramsG = netG.parameters()
...
for i, (org,atb,mask,filt,psf) in enumerate(train_loader):
    ...
    if wave_vcc:
        ...
        for epoch in range(num_epoch):
            ...
            t_start = time.time()
            ksp_H = netG(eta)
            kernel_H = netK(zeta) # 生成csm2
            ksp_H = FCNN(ksp_H, kernel_H, kernel_size) # 生成csm2对应多通道
            kspw = r2c(ksp_H)
            kspw = ifftyc(kspw)
            kspw = kspw * psf
            kspw = fftyc(kspw)
        ...
        for epoch in range(num_epoch):
            ...
            phi = netP(zeta)
            ksp = FCNN(ksp_H, phi, phi_size)
            ksp = 0.8*ksp + 0.2*c2r(torch.conj(torch.flip(torch.flip(r2c(ksp_H), [2]), [3])))
            kernel = netK(zeta)
            kernel_H = netK2(zeta) 
            ksp = FCNN(ksp, kernel, kernel_size) 
            ksp_H = FCNN(ksp_H, kernel_H, kernel_size) 
            ksp = c2r(torch.cat([r2c(ksp), r2c(ksp_H)],1)) 
            loss = criterion(c2r(r2c(ksp)*mask*filt), c2r(r2c(atb)*mask*filt))
            loss.backward()
            optim.step()
            t_end = time.time()
            T = T + t_end - t_start
