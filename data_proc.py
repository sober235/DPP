...
def getTestingData_Wave_com(nImg=1, wave=1, vcc=0, wave_vcc=0, size=[256, 250], device=0): # k-space
    ...
    if wave_vcc:
        ...
        B = mat73.loadmat('/data/data42/LiuCongcong/Qiu_Matlab_Project/LORAKS_v2/examples/Jia_Reduced_Fov/mask_vd_256x250_4x_acs24.mat')
        mask = B['mask'][:]
        mask = mask.astype(np.complex64)
        mask = np.tile(mask, [nImg, 24, 1, 1])

        C = scio.loadmat('/data/data42/LiuCongcong/Qiu_Matlab_Project/LORAKS_v2/examples/Jia_Reduced_Fov/filt_256x250.mat')
        h = C['weight'][:]
        h = h.astype(np.complex64)
        h = np.tile(h, [nImg, 1, 1])
        h = np.expand_dims(h, 0)
        ...
        D = mat73.loadmat('/data/data42/LiuCongcong/Qiu_Matlab_Project/LORAKS_v2/examples/Jia_Reduced_Fov/PsfY_crop.mat')
        f = D['PsfY'][:] # 256x250x24
        f = np.transpose(f, [2, 0, 1]) # 24x256x250
        f = np.expand_dims(f, 0)
        f = f.astype(np.complex64)

        k_phase = scio.loadmat('/data/data42/LiuCongcong/Cui_Zhuoxu/PF_bayes/data/phase_e_k_norm.mat')
        k_phase = k_phase['phase_e_k_norm']
        ...
        toc()
        print('Data prepared!')
        return orgk, atb, mask, h, f, k_phase
    elif vcc:
        A = scio.loadmat('/data/data42/LiuCongcong/wave_dgm_v2/Kdgm-VW/Qiu_data/HighRes/DATA.mat')
        org = A['DATA']
        org = np.transpose(org, [2, 0, 1])
        org = np.expand_dims(org, 0)
        org = np.concatenate([org, np.conjugate(np.flip(np.flip(org, 2), 3))], 1)

        B = mat73.loadmat('./mask/pf_336x338_5x_acs20.mat')
        mask = B['mask'][:]
        mask = mask.astype(np.complex64)
        mask = np.tile(mask, [nImg, 24, 1, 1])
        mask = np.concatenate([mask,(np.flip(np.flip(mask, 2), 3))], 1)

        C = mat73.loadmat('/data/data42/LiuCongcong/wave_dgm_v2/Kdgm-VW/Qiu_data/HighRes/filter_336x338.mat')
        h = C['weight'][:]
        h = h.astype(np.complex64)
        h = np.tile(h, [nImg, 1, 1])
        h = np.expand_dims(h,0)

        D = mat73.loadmat('/data/data42/LiuCongcong/wave_dgm_v2/Kdgm-VW/Qiu_data/HighRes/PsfY_1_2_cycle7.mat')
        f = D['PsfY'][:]
        f = np.transpose(f, [2, 0, 1])
        #f = np.tile(f, [1, 24, 1, 1])
        f = np.expand_dims(f, 0)
        f = f.astype(np.complex64)
        f = np.concatenate([f, np.conjugate(np.flip(f, 2))], 1)
